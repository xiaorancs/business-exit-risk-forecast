...
def runLgbStack(inputfile,outputfile):
    ...
    gbm.fit(X_train11,Y_label11,eval_metric='auc',verbose=5)
    column = ['STACKFEATURE'+str(i) for i in range(50)]

    df_new_feature11 = pd.DataFrame(gbm.apply(df_all_prov11[features]),columns=column)
    ...
    gbm.fit(X_train12,Y_label12,eval_metric='auc',verbose=5)
    column = ['STACKFEATURE'+str(i) for i in range(50)]

    df_new_feature12 = pd.DataFrame(gbm.apply(df_all_prov12[features]),columns=column)
...
outputfile = ['../data/alldata/df_data_all_prov_lgbstack.csv','../data/alldata/df_data_onehot_prov_lgbstack.csv',
             '../data/alldata/df_data_plus_all_prov_lgbstack.csv','../data/alldata/df_data_plus_onehot_prov_lgbstack.csv']
