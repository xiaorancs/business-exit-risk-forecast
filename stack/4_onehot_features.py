...
def runOneHot(inputfile,outputfile):
    '''
    输入输出文件，文件名主义其格式的不同。
    '''
    df_all = pd.read_csv(inputfile)

    df_all = onehotFeature(df_all)
...
outputfile = ['../data/alldata/df_data_onehot.csv','../data/alldata/df_data_plus_onehot.csv']
for i in range(2):
    print(i," start ",inputfile[i])
    runOneHot(inputfile[i],outputfile[i])
